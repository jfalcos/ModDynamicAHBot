...
namespace ModDynamicAH
{
    ...
    inline constexpr char const *CFG_SETUP_ACC_PASS = "ModDynamicAH.Setup.AccountPassword";
    ...
    inline constexpr char const *CFG_SETUP_ALLI_RACE = "ModDynamicAH.Setup.Alliance.Race";
    inline constexpr char const *CFG_SETUP_ALLI_CLASS = "ModDynamicAH.Setup.Alliance.Class";
    inline constexpr char const *CFG_SETUP_ALLI_GENDER = "ModDynamicAH.Setup.Alliance.Gender";
    inline constexpr char const *CFG_SETUP_HORD_RACE = "ModDynamicAH.Setup.Horde.Race";
    inline constexpr char const *CFG_SETUP_HORD_CLASS = "ModDynamicAH.Setup.Horde.Class";
    inline constexpr char const *CFG_SETUP_HORD_GENDER = "ModDynamicAH.Setup.Horde.Gender";
    inline constexpr char const *CFG_SETUP_NEUT_RACE = "ModDynamicAH.Setup.Neutral.Race";
    inline constexpr char const *CFG_SETUP_NEUT_CLASS = "ModDynamicAH.Setup.Neutral.Class";
    inline constexpr char const *CFG_SETUP_NEUT_GENDER = "ModDynamicAH.Setup.Neutral.Gender";

    // Seller / general toggles
    inline constexpr char const *CFG_ENABLE_SELLER = "ModDynamicAH.Enabled";
    inline constexpr char const *CFG_DRYRUN = "ModDynamicAH.DryRun";
    inline constexpr char const *CFG_INTERVAL_MIN = "ModDynamicAH.Interval.Minutes";
    inline constexpr char const *CFG_MAX_RANDOM = "ModDynamicAH.Random.MaxPerCycle";
    ...
    inline constexpr char const *CFG_ALLOW_Q_POOR = "ModDynamicAH.AllowQuality.Poor";
    inline constexpr char const *CFG_ALLOW_Q_NORMAL = "ModDynamicAH.AllowQuality.Normal";
    inline constexpr char const *CFG_ALLOW_Q_UNCOMMON = "ModDynamicAH.AllowQuality.Uncommon";
    inline constexpr char const *CFG_ALLOW_Q_RARE = "ModDynamicAH.AllowQuality.Rare";
    inline constexpr char const *CFG_ALLOW_Q_EPIC = "ModDynamicAH.AllowQuality.Epic";
    inline constexpr char const *CFG_ALLOW_Q_LEGENDARY = "ModDynamicAH.AllowQuality.Legendary";
    inline constexpr char const *CFG_WHITE_ALLOW = "ModDynamicAH.WhiteAllowList";
    inline constexpr char const *CFG_NEVER_ABOVE_VENDOR_BUY = "ModDynamicAH.NeverBuyAboveVendorBuyPrice";
    ...
    inline constexpr char const *CFG_CONTEXT_MAX_PER_BRACKET = "ModDynamicAH.Context.MaxPerBracket";  // default 4
    inline constexpr char const *CFG_CONTEXT_WEIGHT_BOOST = "ModDynamicAH.Context.RecipeWeightBoost"; // default 1.5
    inline constexpr char const *CFG_CONTEXT_VENDOR_SKIP = "ModDynamicAH.Context.SkipVendorMats";     // default true
    ...
    inline constexpr char const *CFG_SCARCITY_PER_TICK_ITEM_CAP = "ModDynamicAH.Scarcity.PerItemPerTickCap";

    // Vendor pricing floor
    inline constexpr char const *CFG_VENDOR_MIN_MARKUP = "ModDynamicAH.Vendor.MinMarkup";
    inline constexpr char const *CFG_VENDOR_CONSIDER_BUYPRICE = "ModDynamicAH.Vendor.ConsiderBuyPriceAsSold";
    ...
    inline constexpr char const *CFG_DEBUG_CONTEXT_LOGS = "ModDynamicAH.Debug.ContextLogs";
    ...
    inline constexpr char const *CFG_CAP_FAMILY_PREFIX = "ModDynamicAH.Cap.Family."; // +Name
    ...
    struct PostRequest
    {
        House house = House::Neutral;
        uint32_t itemId = 0;
        uint32_t count = 1;
        uint32_t startBid = 0;
        uint32_t buyout = 0;
        uint32_t duration = 24 * 60 * 60; // seconds
    };

    // Simple queue (single producer/consumer on main thread)
    class PostQueue
    {
    public:
        void Push(PostRequest &&req) { _q.emplace_back(std::move(req)); }
        size_t Size() const { return _q.size(); }

        std::vector<PostRequest> Drain(size_t max)
        {
            if (max >= _q.size())
            {
                std::vector<PostRequest> out = std::move(_q);
                _q.clear();
                return out;
            }
            std::vector<PostRequest> out;
            out.reserve(max);
            auto itEnd = _q.begin() + max;
            std::move(_q.begin(), itEnd, std::back_inserter(out));
            _q.erase(_q.begin(), itEnd);
            return out;
        }
...
// Shared helper (header-only) to parse CSV list of uint32
inline std::unordered_set<uint32_t> ParseCsvU32(std::string const &csv)
{
    ...
    while (off < csv.size())
    {
        size_t next = csv.find_first_of(",;", off);
        std::string num = csv.substr(off, next == std::string::npos ? next : next - off);
        char *end = nullptr;
        uint32_t v = static_cast<uint32_t>(std::strtoul(num.c_str(), &end, 10));
        if (end != num.c_str())
            out.insert(v);
